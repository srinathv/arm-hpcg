...
#pragma omp parallel for
	...
	for ( local_int_t i = 0; i < localNumberOfRows; i++ ) {
		local_int_t ix = i % nx;
		local_int_t iz = i / (nx*ny);
		local_int_t iy = (i / nx) - (iz*ny);

		local_int_t giz = giz0 + iz;
		local_int_t giy = giy0 + iy;
		local_int_t gix = gix0 + ix;

		local_int_t currentGlobalRow = gix + giy*gnx + giz*gnx*gny;
		...
		// Check all the neighbours (max of 27, or numberOfNonzerosPerRow). Do NOT do it with a three-level loop for x,y,z
		// We do this by creating an imaginary 3x3x cube with currentLocalRow on position (0,0,0)
		const local_int_t nSizeX = 3;
		const local_int_t nSizeY = 3;
		const local_int_t nSizeZ = 3;
		for ( local_int_t j = 0; j < numberOfNonzerosPerRow; j++ ) {
			local_int_t sx = j % nSizeX;
			local_int_t sz = j / (nSizeX*nSizeY);
			local_int_t sy = (j / nSizeX) - (sz*nSizeY);
			// Convert to [-1,1] displacements on each axis
			sx--; sy--; sz--;

			// Check boundaries before doing anything:
			// 	sx+gix cannot be less than 0 or more than gnx
			// 	sy+giy cannot be less than 0 or more than gny
			// 	sz+giz cannot be less than 0 or more than gnz
			local_int_t xx = sx+gix;
			local_int_t yy = sy+giy;
			local_int_t zz = sz+giz;
			if ( zz >= 0 && yy >= 0 && xx >= 0 && zz < gnz && yy < gny && xx < gnx ) {
				global_int_t curCol = currentGlobalRow + sx + sy*gnx + sz*gnx*gny;
