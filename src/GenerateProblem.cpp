...
void GenerateProblem(SparseMatrix & A, Vector * b, Vector * x, Vector * xexact) {
  ...
  int total_size = localNumberOfRows * numberOfNonzerosPerRow;
  ...
  for ( local_int_t i = 0; i < localNumberOfRows; i++ ) {
	mtxIndL[i] = templ + offset;
	matrixValues[i] = tempd + offset;
	mtxIndG[i] = tempg + offset;
	offset += numberOfNonzerosPerRow;
  }
...
// C++ std::map is not threadsafe for writing
        ...
        HPCG_fout << " rank, globalRow, localRow = " << A.geom->rank << " " << currentGlobalRow << " " << A.globalToLocalMap[currentGlobalRow] << std::endl;
#endif
...
#ifdef HPCG_DETAILED_DEBUG
  HPCG_fout     << "Process " << A.geom->rank << " of " << A.geom->size <<" has " << localNumberOfRows    << " rows."     << std::endl
